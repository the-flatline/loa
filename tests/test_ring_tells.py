...
def _set(monkeypatch, cond):
    monkeypatch.setattr(ringd, "_condition", lambda ttl=1.0: cond)
    ringd._COND["ts"] = 0.0
...
def test_hurts_stops_breathing_and_holds_red(monkeypatch):
    ...
    monkeypatch.setattr(ringd, "_condition", cond)
    monkeypatch.setattr(ringd.time, "sleep", lambda s: None)
    ringd.hurting(ring)
...
def test_condition_reads_well_when_the_body_is_clean():
    assert fault.condition({"ts": __import__("time").time(), "faults": 0,
                             "warns": 0, "rows": []}) == "well"
...
def test_stale_or_missing_sweep_reads_mute_never_well():
    ...
    stale = {"ts": _t.time() - fault.FAULTS_STALE_S - 1,
